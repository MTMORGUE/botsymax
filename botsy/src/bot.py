...
class Bot:
    def __init__(self, name, config_path, port):
        ...
        # Create a storage directory for this bot under "bots/(bot name)"
        self.storage_dir = os.path.join("bots", self.name)
        ...
        # Additional auto flags for other platforms if needed
    ...
    def get_user_id(self, username):
        ...
        if username_lower in self.user_id_cache:
            logging.info(
                f"🔄 Bot {self.name}: Using cached user_id for {username}: {self.user_id_cache[username_lower]}")
        ...
        try:
            ...
            logging.warning(
                f"⚠️ Bot {self.name}: Rate limit reached while fetching user_id for {username}. Counting as task complete.")
            return None
        except Exception as e:
    ...
    def get_user_ids_bulk(self, usernames):
        ...
        if new_usernames:
            try:
                ...
                logging.warning(
                    f"⚠️ Bot {self.name}: Rate limit hit during bulk user lookup. Counting as task complete.")
    ...
    def get_bot_recent_tweet_id(self, cache_duration=300):
        ...
        try:
            ...
            logging.warning(
                f"⚠️ Bot {self.name}: Rate limit reached while fetching bot's recent tweet id. Returning to console.")
            return None
        except Exception as e:
    ...
    def daily_tweet_job(self):
        ...
        for _ in range(MAX_AUTH_RETRIES):
            if self.post_tweet():
                success = True
                break
            # Return immediately so control goes back to the console
            return
        if success:
    ...
    def daily_comment(self):
        ...
        for handle_name in handles:
            ...
            try:
                ...
                logging.warning(
                    f"⚠️ Bot {self.name}: Rate limit hit while fetching tweets for '{handle_name}'. Returning to console.")
                return
            except Exception as e:
        ...
        if os.path.exists(self.engagement_metrics_file):
            try:
                with open(self.engagement_metrics_file, "r") as f:
                    metrics = json.load(f)
                dashboard.append(
                    f"Last Tweet - Likes: {metrics.get('likes', 0)}, Retweets: {metrics.get('retweets', 0)}")
            except Exception as e:
